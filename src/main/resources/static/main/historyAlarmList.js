...
var condition={}
//取消清除
function redoCleanAlarm(parm){
	  $.post("/alarm/cleanAlarm",
			  {"alarmId":parm,"userId":"1","isClean":false},function(data){
				  if(data == 'true'){
...
jQuery(function($) {
  ...
  jQuery(grid_selector).jqGrid({
   ...
   jsonReader : { 
    ...
    url: "/alarm/findHistoryAlarms",
    datatype: "json",
    postData:condition,
    mtype: 'POST',
    colNames:['alarmId','级别','名称','告警源类型', '告警源', '具体定位','告警时间','清除时间','确认人','清除人','操作','备注'],
    colModel:[
      ...
      {name:'clearTime',index:'clearTime'},
      ...
      {name:'myac',index:'',editable:false,
    	    formatter: function (value, grid, rows, state) {
    	        return "<a onclick=\"direct("+rows.alarmId+")\">跳转   </a>"
    	        		+"<a onclick=\"redoCleanAlarm("+rows.alarmId+")\">取消清除  </a>"
    	        		+"<a onclick=\"remarkAlarm("+grid.rowId+")\">备注  </a>";
    	    }
        },
      {name:'remark',index:'remark',editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}}
      ],
    width:"80%",
    autowidth:true,
    height:"100%",
  ...
  */
  ...
  $("#cleans").click(function(){
	  ...
	  alarmIds=alarmIds.substring(0,alarmIds.length-1)
  	  $.post("/alarm/cleanAlarm",
  			{"alarmId":alarmIds,"isClean":false},function(data){
  				if(data == 'true'){
...
$("#dump").click(function(){
        ...
        form.append($("<input></input>").attr("type", "hidden").attr("name", "type").attr("value", "history"));
